...
**平台：** Cisco IOS / GNS3  
**關鍵技術：** Multi-Routing Protocols (EIGRP, OSPF, BGP), Route Redistribution, GRE over IPsec VPN[cite: 1]
...
### 傳統問題
異構網路（多路由協定）難以互通，且跨公網傳輸缺乏安全加密與動態路由支援[cite: 1]。

### 解決方案與核心亮點
* **高可用性與擴充性 (Availability & Scalability)：** 企業包含多個自治系統與區域（EIGRP AS 100, OSPF Area 0, BGP AS 64512/64513），傳統點對點靜態設定擴充困難[cite: 1]。本專案透過邊界路由器雙向路由重分發 (Route Redistribution) 實現全網動態互通，並搭配 GRE 隧道架構封裝動態路由協定封包[cite: 1]。
* **安全性 (Security)：** 純 GRE 隧道缺乏加密能力（明文傳輸），為了保護敏感企業資料，疊加 IPsec (IKEv1/ESP) 進行機密性保護[cite: 1]。同時關閉路由器與交換器上未使用的協定及遺留功能（如關閉未使用的介面）以增加安全性與防範網路攻擊[cite: 1]。
...
H   Address         Interface       Hold Uptime   SRTT   RTO  Q  Seq
                                    (sec)         (ms)       Cnt Num
1   20.1.0.2        Gi0/1           12   00:04:02    1   100   0   11[cite: 1]
0   10.1.0.1        Gi0/0           14   00:19:24    1   100   0   11[cite: 1]


2. OSPF Neighborship
OSPF-1#show ip ospf neighbor
Neighbor ID     Pri   State           Dead Time   Address         Interface
50.1.0.1          1   FULL/BDR        00:00:33    40.1.0.2        GigabitEthernet0/1[cite: 1]
30.1.0.1          1   FULL/DR         00:00:36    30.1.0.1        GigabitEthernet0/0[cite: 1]
...
C        10.1.0.0/30 is directly connected, GigabitEthernet0/0[cite: 1]
L        10.1.0.1/32 is directly connected, GigabitEthernet0/0[cite: 1]
      20.0.0.0/16 is subnetted, 1 subnets
D        20.1.0.0 [90/15368] via 10.1.0.2, 02:06:55, GigabitEthernet0/0[cite: 1]
      30.0.0.0/30 is subnetted, 1 subnets
D EX     30.1.0.0 [170/1034240] via 10.1.0.2, 01:51:33, GigabitEthernet0/0[cite: 1]
      40.0.0.0/30 is subnetted, 1 subnets
D EX     40.1.0.0 [170/1034240] via 10.1.0.2, 01:50:27, GigabitEthernet0/0[cite: 1]
