...
```text
+----------------+          +----------------+          +----------------+
|    EIGRP-1     |          |    EIGRP-2     |          |   EIGRP-OSPF   |
| (192.168.100.1)|          |                |          |                |
+-------+--------+          +-------+--------+          +-------+--------+
        | (g0/0)                    | (g0/0)                    | (g0/1)
        | .1                        | .2                        | .2
        |                           |                           |
        +------ (10.1.0.0/30) ------+------ (20.1.0.0/30) ------+
                                                                |
                                                                | (g0/0) .1
                                                                |
                                                        (30.1.0.0/30)
                                                                ...
                                                                | (g0/0) .2
+----------------+          +----------------+          +-------+--------+
|     BGP-1      |          |    OSPF-BGP    |          |     OSPF-1     |
| (192.168.100.2)|          |                |          |                |
+-------+--------+          +-------+--------+          +-------+--------+
        | (g0/1)                    | (g0/1)                    | (g0/1)
        | .2                        | .1                        | .1
        |                           |                           |
        +------ (50.1.0.0/30) ------+------ (40.1.0.0/30) ------+
...
完整文件：有關完整的設計說明和完整的路由器配置，請查看上傳到此儲存庫中的原始專案文件 --> 作品 3 -3： 企業級多協定路 (Multi-Routing Protocols) 由重分發 ( Redistribution ) 與 GRE over IPsec 安全 VPN 整合實
